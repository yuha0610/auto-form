...
| `npm run screen:starcla` | スタクラに求人掲載中の企業を「送信NG」にする | `--apply` で書き込み |
...
スタクラ(`startupclass.co.jp`)に求人を掲載している企業は、すでに接点があるため
フォーム営業の対象から外したい。このスクリプトは求人一覧から掲載企業を取得し、
シートの企業名と突き合わせて備考に「送信NG」を追記する。行の削除は一切しない。

1. ドライラン実行して対象一覧を確認する:
   ```bash
   npm run screen:starcla
   ```
   求人一覧の全ページを0.7秒間隔で取得する(1〜2分程度)。取得した掲載企業は
   `data/starcla-companies.json` に保存され、あとから「何を根拠に送信NGにしたか」を追える。
   ページの取得に3回失敗した時点で中断する(一部だけ取り落として、掲載中なのにNGが
   付かない企業が黙って残るのを防ぐため)。
...
企業名の突き合わせは法人格の表記ゆれを吸収したコア名一致で行う(`cleanup:company-names`と同じ
ロジック)。スタクラの社名欄には「Landit Inc. / ランディット株式会社」のような併記や、
キャッチコピー・旧社名が混ざって入っていることがあるため、それらを落とした別名でも照合する。同じコア名の行が複数ある場合はその全てが対象になるので、一覧の目視確認を省略しないこと。
